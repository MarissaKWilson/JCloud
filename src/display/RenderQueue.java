...
public class RenderQueue {
	...
	public BufferedImage call() throws VisualizerConfigException, IOException {
		// weights = new GitParser(new File(srcTree.getAbsolutePath() +
		// "/.git"),
		...
		Point2D.Double parentNetworkPoint = new Point2D.Double(3 * width / 4,
				3 * height / 4);
		ParentNetworkPlacement parentNetworkPlacementTmp = new ParentNetworkPlacement(
				glyphs, parentNetworkDimension, parentNetworkPoint, graph);
		SpiralIterator spiralTmp = new SpiralIterator(spiralMaxRadius,
				spiralSteps, squashdown);
		CenteredTokenWrapper centeredTokenTmp = new CenteredTokenWrapper(
				parentNetworkPlacementTmp);// , spiralTmp, new
											// JavaColorScheme(rand, 20));

		LayoutTokens layoutTokens = new LayoutTokens(width, height, maxTokens,
				fontTransformer, awtIntersectorTmp, centeredTokenTmp,
				spiralTmp, new JavaColorScheme(rand, 20, graph));
...
//		Iterator<iToken> authorIterator = authors.iterator();
//		iToken tmpAuthor = new Author(" ");
